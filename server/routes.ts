...
export async function registerRoutes(
  ...
  app.post("/api/run-whamo", (req, res) => {
    ...
    // we can also fallback to the uploaded sample if the engine is not fully setup
    if (fs.existsSync(exePath)) {
      execFile("wine", [exePath, inpPath, outPath], (error, stdout, stderr) => {
        if (error) {
          console.error("WHAMO Error:", error);
          console.error("stderr:", stderr);
          return res.status(500).send(`WHAMO execution failed: ${stderr || error.message}`);
        }

        if (!fs.existsSync(outPath)) {
          return res.status(500).send("OUT file not generated by WHAMO.");
        }
      ...
      // This allows the "Generate .out" button to work for the user immediately
